...
typedef struct  __attribute__((packed)) 
{
	u8 enable;  			// 是否处于激活状态  0：失效状态  1：使能状态
	u16 n_1ms;		    // 该队列 运行时间，定义运行时间变量，最小分辨率为1ms
	u16 period_ms;		// 该队列 连续发送周期，指所要求的定时发送周期
	u16 regaddr;		// 寄存器变量起始地址 
	u8 length;			// 数据长度
	u8 objAddr;			// 发送目标地址
	char header;          //(方)添加，用于判断将要打包发送的数据属于哪个数据文件中
	// struct USART_TypeDefStruct *pUSART; // 串口号
	struct Repeat_type *next;
} Repeat_type;
